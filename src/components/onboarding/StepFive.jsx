  ...
  return (
    ...
    >
      <div className="max-w-[680px] mx-auto flex flex-col gap-6 sm:gap-8">
        ...
        {/* STEP */}
        <p className="text-[#186AFF] text-[14px] sm:text-[16px] font-medium">
          STEP 5 OF 5
        </p>

        {/* HEADER */}
        <div className="flex flex-col gap-2">
          <h2 className="text-[20px] sm:text-[24px] font-medium text-black">
            Rate
          </h2>
        ...
        >
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <button className="h-11 px-5 rounded-xl border border-black/20 flex items-center gap-2 text-[16px] sm:text-[18px] font-medium text-black/60">
              ← Back
            </button>

            <span className="hidden sm:block h-6 w-px bg-black/20" />

            <button className="text-[16px] sm:text-[18px] font-medium text-black/60">
              Skip for now
            </button>
          </div>

          {/* RIGHT */}
          <button className="h-11 px-6 rounded-xl bg-[#186AFF] text-white text-[16px] sm:text-[18px] font-medium">
            Finish Setup
          </button>
        </div>
...
function RateCard({ title, unit }) {
  return (
    <div className="flex flex-col gap-2">
      <p className="text-[14px] sm:text-[16px] text-black">{title}</p>
...
function RateRow({ label, unit, disabled }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
      <p className="text-[13px] sm:text-[14px] text-black/60">{label}</p>

      <div
        className={`h-8 px-3 rounded-lg flex items-center gap-1 w-fit
      ...
      >
        ...
        <input
          ...
          disabled={disabled}
          placeholder="0.00"
          className="w-14 bg-transparent outline-none text-[16px] text-black text-right"
        />
        <span>{unit}</span>
      </div>
    </div>
  ...
  return (
    <div className="h-8 px-3 rounded-lg bg-white border border-black/25 flex items-center gap-1">
      ...
      <input
        ...
        placeholder="0.00"
        className="w-14 bg-transparent outline-none text-[16px] text-black text-right appearance-none"
      />
      <span>/sq.ft.</span>
    </div>
  );
