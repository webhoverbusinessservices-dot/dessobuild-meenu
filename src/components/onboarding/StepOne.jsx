...
const StepOne = ({ currentStep, setCurrentStep }) => {
  ...
  return (
    <section className=" bg-white rounded-3xl border border-[rgba(24,106,255,0.16)] shadow-[0px_186px_74px_rgba(0,0,0,0.01),0px_105px_63px_rgba(0,0,0,0.04),0px_46px_46px_rgba(0,0,0,0.07),0px_12px_26px_rgba(00.08)] p-6 sm:p-8 lg:p-10 w-full">
      {/* INNER WIDTH */}
      <div className="max-w-[680px] mx-auto flex flex-col gap-6 sm:gap-8">
        ...
        <div className="flex flex-col gap-6 sm:gap-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[201px_1fr] gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                type="button" className=" h-10 rounded-xl border border-[#186AFF] text-[#186AFF] font-switzer font-medium text-[14px] sm:text-[16px] hover:bg-blue-50 " >
          ...
          {/* ADDRESS / PIN */}
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_184px] gap-4">
            <Input label="Address *" />
            <Input label="Pincode *" />
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-6 sm:pt-8 border-t border-black/10">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <button
              onClick={handleBack} className=" h-11 px-5 rounded-xl border border-black/20 flex items-center gap-2 text-[16px] sm:text-[18px] font-medium text-black/60 hover:bg-black/[0.02] " >
              ← Back
            </button>
          ...
          {/* RIGHT */}
          <button
            onClick={handleNext}  className=" h-11 px-6 rounded-xl bg-[#186AFF] text-white text-[16px] sm:text-[18px] font-medium hover:bg-blue-700 "  >
            Save & Continue
          </button>
        </div>
      </div>
...
const Input = ({ label, type = "text" }) => (
  <label className="flex flex-col gap-1 font-switzer text-[14px] sm:text-[16px] text-black">
    {label}
    <input
      type={type}  className=" h-10 px-3 rounded-lg border border-black/25 bg-white focus:outline-none focus:ring-2 focus:ring-[#186AFF]"
    />
  </label>
);
